...
const MODES = {
    ...
    'itt': {
        inputs: [
            ...
            { id: 'p2', label: 'Prop. Group 2 (Treatment) %', type: 'range', min: 0.1, max: 99.9, step: 0.1, val: 40, desc: 'Anticipated outcome in Treatment Group.' },
            { id: 'c1', label: 'Drop-in Rate (%)', type: 'range', min: 0, max: 50, step: 1, val: 5, desc: 'Controls who take the treatment.' },
            { id: 'c2', label: 'Drop-out Rate (%)', type: 'range', min: 0, max: 50, step: 1, val: 10, desc: 'Treated who stop the treatment.' },
            { id: 'power', label: 'Power (%)', type: 'range', min: 80, max: 99, val: 80, desc: '' },
            { id: 'confidence', label: 'Confidence Level (%)', type: 'range', min: 90, max: 99, val: 95, desc: '' },
            { id: 'ratio', label: 'Group Ratio (N1/N2) (r)', type: 'number', val: 1, desc: '' }
        ],
        formulaStr: `
            <div style="font-size:0.9em; line-height:1.4">
                <strong>Base N (per group):</strong> N<sub>base</sub><br>
                <strong>ITT Adjustment:</strong> N<sub>ITT</sub> = N<sub>base</sub> / (1 - c<sub>1</sub> - c<sub>2</sub>)<sup>2</sup><br>
                <div style="font-size:0.8em; margin-top:5px; opacity:0.8">
                    c<sub>1</sub> = Drop-in rate, c<sub>2</sub> = Drop-out rate.
                </div>
            </div>
        `,
        formulaSteps: '', // Will be dynamic
        interpretation: `
            <p>Calculates Sample Size for Intention-To-Treat (ITT) analysis where non-adherence (drop-ins and drop-outs) attenuates the observed effect. The base RCT sample size is inflated to maintain power.</p>
        `,
        calc: (state) => {
            const P1 = parseFloat(state.p1) / 100; // Control / Group 1
            const P2 = parseFloat(state.p2) / 100; // Treatment / Group 2
            const r = parseFloat(state.ratio) || 1;
            const c1 = parseFloat(state.c1) / 100;
            const c2 = parseFloat(state.c2) / 100;
            const power = parseInt(state.power);
            const conf = parseInt(state.confidence) || 95;
            const alpha = (100 - conf) / 100;
            const beta = (100 - power) / 100;
            ...
            if ((c1 + c2) >= 1) {
                return { n: 'Error', display: 'Total non-adherence must be < 100%.', table: null };
            }

            // Calculate Base RCT sizes without dropout buffer
            // P2 = Treatment, P1 = Control
            const baseResults = calculateAdvanced(P2, P1, r, alpha, beta, false);

            // Adjust for ITT
            const adjustmentFactor = Math.pow(1 - c1 - c2, 2);

            const adjustObj = (obj) => ({
                n1: Math.ceil(obj.n1 / adjustmentFactor),
                n2: Math.ceil(obj.n2 / adjustmentFactor),
                total: Math.ceil(obj.total / adjustmentFactor)
            });

            const ittResults = {
                kelsey: adjustObj(baseResults.kelsey),
                fleiss: adjustObj(baseResults.fleiss),
                fleiss_cc: adjustObj(baseResults.fleiss_cc)
            };

            let displayStr = `P1=${(P1 * 100).toFixed(2)}% P2=${(P2 * 100).toFixed(2)}% Drop-in=${(c1 * 100).toFixed(0)}% Drop-out=${(c2 * 100).toFixed(0)}%`;

            return {
                n: ittResults.fleiss.total,
                display: displayStr,
                table: ittResults,
                visualData: {
                    n1: ittResults.fleiss.n2, // n2 output from helper = Group 1 Control
                    n2: ittResults.fleiss.n1, // n1 output from helper = Group 2 Treatment
                    label1: 'Control (ITT)',
